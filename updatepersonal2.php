...
$subsubcat=mysql_real_escape_string($_POST['subsubcategory']);
$q=mysql_query("select * from education where ID = '" . $_SESSION['ID'] . "' and course='XII'");
$count=mysql_num_rows($q);
if($count==1)
{
$query="UPDATE register_emp set firstname='$f',lastname='$l',gender='$g',birthdate='$d',city='$city',phone='$phone',category='$category',subcategory='$subcat',subsubcategory='$subsubcat' WHERE ID = '" . $_SESSION['ID'] . "'";
$data=mysql_query($query,$dbc) or die(mysql_error());
}
else
{
$query="insert into register_emp(ID,firstname,lastname,gender,birthdate,city,phone,category,subcategory,subsubcategory) values ('" . $_SESSION['ID'] . "','$f','$l','$g','$d','$city','$phone','$category','$subcategory','$subsubcategory')";
$data=mysql_query($query,$dbc) or die(mysql_error());
}
}
//add XII details
...
if($count==1)
{
$query="UPDATE education set cname='$cn',institution='$a',university='$b',fromyear='$c',toyear='$d',marks='$e',field='$f' WHERE ID = '" . $_SESSION['ID'] . "' and course='postgraduation'";
$data=mysql_query($query,$dbc) or die(mysql_error());
}
else
{

...
<?php 
$q="select * from recommend,register_emp where recommend.ID=register_emp.ID and recommend.jobid='".$_GET['jobid']."' and status='selected' ORDER BY register_emp.experience desc";
$data=mysql_query($q,$dbc) or die(mysql_error());
$recc=mysql_num_rows($data);
echo '<center>Number of Selected Candidates:  '.$recc.'</center>';
while($row=mysql_fetch_array($data)) 
{
?>    
    <div class="entry">
...
<br/>
<input type="submit" name="sesubmit"></input>
</form>
  </div>
         </div>
<?php } ?>

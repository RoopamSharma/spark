...
<?php 
    ...
    $query = "SELECT * FROM userresult WHERE user_id= '" . $_SESSION['email'] . "'";
...
<?php   if (mysql_num_rows($data) == 0) {
    ...
    <div class="alert alert-dismissible alert-info">
      ...
      <!--do put a alert before test_display.php before test if user haven't haven filled the test information-->
        <strong>Oh snap!</strong>  You havent taken a test. <a href="test_display.php" class="alert-link" align='center'> Take one !!</a></div> 
  <?php }	?>
  <!--warning ends here-->
  <div class="gradientBoxesWithOuterShadows container panel panel-primary jumbotron " style="padding-top:0px;border-color:black;font-family: 'Titillium Web', sans-serif;">
  <h2 class="page-header">Your Profile</h2>
  <div class="row" style="padding-top:20px;">
    <!-- left column -->
    <div class="col-md-4 col-sm-8 col-xs-12" style="height: 300px;margin-left:10%">
...
<div class="col-md-6 col-sm-8 col-xs-12" style="margin-left:2%" >
...
<?php } 
  ...
  <a href="#" class="btn btn-default" onclick="alert('CV not present!');">View Your CV</a>
<?php } ?>
    <!--button to show cv-->
     
</div>
<div>
        <label>  <a href="#" data-toggle="modal" data-target="#login-modal">Edit Your Personal Details </a>
        </label>
 </div>
        ...
        <label>About me&nbsp;:&nbsp;&nbsp; </label><?php echo $emp['aboutme']; ?>
         <br/>
        <label><a href="#" data-toggle="modal" data-target="#about-modal">Edit About</a>
         </label>
...
<?php
	 ...
	 $abcd=mysql_num_rows($data);
	 $edu=mysql_fetch_assoc($data);
	echo ' <h4><strong>Class XII</strong></h4>';
    if ($abcd==0) { ?>
     <label><a href="#" data-toggle="modal" data-target="#twelve-modal">Add Class XII details</a></label>
     <?php } else { ?>     
		<div >
        <div>
        <br/>
        ...
        <div>
     <div ><label><a href="#" data-toggle="modal" data-target="#twelve-modal">Edit</a></div>
         </div></label>
	 ...
	 $abcd=mysql_num_rows($data);
	 $edu=mysql_fetch_assoc($data);
echo ' <h4><strong>Graduation</strong></h4>';
        
      if ($abcd==0) { ?>
<label>     <a href="#" data-toggle="modal" data-target="#grad-modal">Add Graduation details</a>
     </label><?php } else { ?>     
        ...
        <div ><label><a href="#" data-toggle="modal" data-target="#grad-modal">Edit</a></label></div>
         <?php } ?>
        </div>
      </div>
	 ...
	 $abcd=mysql_num_rows($data);
	 ?>
    <div class="col-md-6 col-sm-8 col-xs-12" style="float:right;margin-right:5%">
                  <div >
...
<label>
    <a href="#" data-toggle="modal" data-target="#pgadd-modal">Add Post Graduation details</a>
        ...
        <br/>        <label>Percentage&nbsp;:&nbsp;&nbsp;</label><?php echo $edu['marks'];?>
         <div>
     <div><label><a href="#" class="pg" data-id=<?php echo $edu['edu_id']?> data-toggle="modal" data-target="#pg-modal">Edit</a></div>
         </label></div>
        <?php } ?>
        </div>
...
<div class="modal fade" id="editcerti-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
        <div class="loginmodal-container">
          <h1>Edit Certifications and Courses</h1><br>
        ...
        <button type="submit" class="btn btn-submit" id="certiadd" name='addsubmit' value="save">Save</button>
        <button type="button" class="btn btn-primary" data-dismiss="modal">Cancel</button>
       <!--<input type="button" id="submit" name="submit" value="Submit"></input>-->

     <!--removed div-->
<input type='hidden' name="hd" id="hd" value=""/> in certiedit modal before 
    </form> 
        </div>
      </div>
      </div>   
...
?>
<label>
<a href="#"  data-toggle="modal" data-target="#job-modal">Add Previous Job details</a></label>
...
<div>
  <div>
          ...
          <br/>
          <div><label><a href="#" class="anchor" data-id=<?php echo $job['proid']?> data-toggle="modal" data-target="#job1-modal">Edit</a></div>
      </label>  
        </div>
...
<div style="margin-top:60px">
<h4><strong>Certifications and Courses</strong></h4>
<label><a href="#" data-toggle="modal" data-target="#certi-modal">Add Certifications and Courses</a>    
    </label><?php $q="select * from education where ID='".$_SESSION['ID']."' and course='certification'";
   $data=mysql_query($q,$dbc) or die(mysql_error());
   $abcd=mysql_num_rows($data);
    while($edu=mysql_fetch_assoc($data)){
  ...
  ?>
     ...
     <div ><label><a href="#" class="anchor" data-id=<?php echo $edu['edu_id'];?> data-toggle="modal" data-target="#editcerti-modal">Edit</a></label></div>
         </div>
    </div>
         <?php } ?>
...
$.ajax({
  type:"POST",
  url:'updatepersonal.php',
  data:$('form.editcerti-modal').serialize(),
  success: alert('Succesas'),
  error: function(){
                alert("failure");
            }
 ...
 //alert("here2");
    ...
    $.ajax({
      ...
      success:function(response){
        ...
        //var company = json["company"];
        //alert(a);
       $("#editcerti-modal #hd").val(myBookId);
       $("#editcerti-modal #cname").val(json["cname"]);
       $("#editcerti-modal #field").val(json["field"]);
       $("#editcerti-modal #institution").val(json["institution"]);
       $("#editcerti-modal #university").val(json["university"]);
       $("#editcerti-modal #fromyear").val(json["fromyear"]);
       $("#editcerti-modal #toyear").val(json["toyear"]);
       $("#editcerti-modal #per").val(json["per"]);
